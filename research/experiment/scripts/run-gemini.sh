#!/usr/bin/env bash
# Run a single task on Gemini CLI.
# Usage: ./run-gemini.sh <task_id> <condition>
# Output: research/experiment/data/gemini/<condition>/task-<id>.json
set -euo pipefail
...
OUTDIR="$ROOT_DIR/research/experiment/data/gemini/${CONDITION}"
mkdir -p "$OUTDIR"
OUTFILE="${OUTDIR}/task-${TASK_ID}.json"

# For Gemini: C3 falls back to C2 (no hook system)
case "$CONDITION" in
  c1)
...
echo "[$(date +%H:%M:%S)] Running: gemini / ${CONDITION} / task-${TASK_ID}"
gemini -p "$FULL_PROMPT" -o json -y --cwd "$CWD" > "$OUTFILE" 2>&1
echo "[$(date +%H:%M:%S)] Done: gemini / ${CONDITION} / task-${TASK_ID} -> $OUTFILE"

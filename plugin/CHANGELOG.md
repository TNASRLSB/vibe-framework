...
Subagent dispatch discipline release. The original Tier B spec (Mnemos parallel memory + Brain CLI) was deprecated 2026-05-02 after consumer-test verification: both proposed features required manual `/vibe:mnemos` / `/vibe:brain` invocations with no autonomous in-session consumer, and a rot-rate audit on the existing auto-memory returned 0 verified rotted citations across 36 entries — the problem Mnemos's citation enforcement would solve does not yet exist at current scale. The replacement spec keeps the *engines* of the original ports (subagent-role formalization from Godspeed/Homer L3 MUSES; severity classifier signal table from Godspeed/Homer L1 Brain) but reshapes them as upgrades to existing dispatch flows. Both features ship without new user commands, new skills, or new hooks — they augment `vibe:audit` subagent dispatch and the CLAUDE.md managed region. See `docs/spec/2026-05-02-vibe-5.9.0-tier-b.md` for the full spec.

### Changes

- **Feature 1 — Subagent role discipline.** Each of the 7 audit-domain agents in `plugin/agents/` (`baptist`, `emmet`, `ghostwriter`, `heimdall`, `orson`, `scribe`, `seurat`) gains four standardized sections after the existing `## Workflow`: `## Tool Discipline` (prose constraints layered over the frontmatter `tools:` allowlist — e.g., seurat's "Bash only for measurement, not arbitrary scripts"), `## Output Format` (fixed markdown structure with required sub-sections — Findings table, role-specific report sections like Token Usage Report for seurat or OWASP Top 10 Coverage for heimdall, conditional Worktree Changes block, optional Suggested Project Rules), `## Boundary Discipline` (explicit do-not rules naming the routing alternative — e.g., baptist's "do not propose copy rewrites — that is ghostwriter's domain"), `## Failure Modes` (3-column table of mode / detection signal / response — e.g., orson's "ffprobe unavailable → file-system metadata only"). New shared template at `plugin/skills/_shared/agent-role-template.md` documents the four-section contract and explains the rationale (without these sections, parallel-dispatch synthesis cannot reliably parse outputs across 7 agents and scope creep produces overlap). `plugin/skills/_shared/audit-protocol.md` gains a one-paragraph cross-reference at the top, pointing each dispatched agent at its own definition file for the role-specific shape. Source: `references/Godspeed/toke/automations/homer/muses/calliope/SKILL.md` (canonical four-section structure adapted for VIBE conventions; `expertise.json` per-agent learning loop deferred to 5.10.0 alongside telemetry). Iron Man fit: today every dispatched audit agent free-forms its output and the `vibe:audit` synthesis step in §Cross-domain correlation has to pattern-match across inconsistent shapes; this turns the contract from prose into structure. Tests: 41 cases (template existence + four section names; each of 7 agents has all 4 sections; each agent's Output Format includes a `## Findings` subsection; audit-protocol cross-reference present).

- **Feature 2 — Dispatch tier guidance (managed CLAUDE.md block).** New file at `plugin/skills/_shared/subagent-dispatch-guidance.md` — markdown rules adapted from `references/Godspeed/toke/automations/local/severity_classifier.py`: 10 normalized signals (prompt length, file refs, code-keyword density, complexity-word density, domain mention, multi-step indicator, reasoning-required indicator, creative-output indicator, risk indicator, token-budget hint), tier mapping table (S0..S5 → haiku/sonnet/opus + effort), 4 floor guardrails (research → S2 minimum, security audits → S4 minimum, irreversible actions → S3 minimum, architectural decisions → S4 minimum) + 2 ceiling guardrails (pure renames → S1 maximum, trivial lookups → S0 maximum), application protocol ("before any Agent tool call: score signals → apply guardrails → set Agent.model parameter; fall back to skill's static model on ambiguity"). New `{{DISPATCH_GUIDANCE_BLOCK}}` placeholder in `plugin/setup/claude-md-template.md` between `Git Signals` and `Verification Discipline`. `plugin/setup/smart-generator.sh` extended to read the guidance file, strip its H1, and emit a new top-level JSON key `dispatch_guidance` capped at 4000 chars (1000 tokens). Total managed-block budget raised 5600 → 9600 chars (1400 → 2400 tokens). `plugin/setup/reconciler.sh` substitution map gains `("dispatch_guidance", "{{DISPATCH_GUIDANCE_BLOCK}}")`. `plugin/scripts/setup-check.sh` gains Check 2d: when CLAUDE.md has the VIBE managed region but the dispatch-guidance section is absent (string `Subagent Dispatch Tier Guidance` not present), emit anomaly `VIBE 5.9.0 dispatch tier guidance missing from CLAUDE.md — run /vibe:setup to refresh the managed region`. Soft anomaly, never blocks. Bypass: `VIBE_NO_DISPATCH_GUIDANCE=1` env var — when set, smart-generator emits an empty block and Check 2d skips. Honest limitation documented in spec §3: this is advisory ambient context, not enforced via hook (CC currently exposes no hook surface that mutates `Agent` tool inputs); empirical pattern from existing managed CLAUDE.md content shows agents follow ambient context reliably. If field signal in 5.10.0+ shows the dispatch tier is being ignored, escalate to a stricter mechanism. Iron Man fit: every Agent dispatch from a VIBE skill or main-agent flow today inherits the session model — no per-subtask tier sizing. The guidance generalizes the per-skill `vibe:spec` classifier pattern across every dispatch surface without new commands. Tests: 8 cases (block non-empty, signal-scoring header, all 6 tiers S0..S5 present, floors+ceilings present, application protocol present, env-var bypass empties block, idempotent re-render, H1 stripped from rendered block).

### Skills

Unchanged at 15. Both 5.9.0 features upgrade existing surfaces (agent definition files, CLAUDE.md managed region, audit-protocol cross-reference) without adding new skills.

### Hook count

Unchanged at 22. 5.9.0 adds capability INSIDE existing surfaces (smart-generator block emission, setup-check Check 2d) without new hook event registrations.

### Tests

391 passing, 0 failed. 5.8.0 baseline 342 → 5.9.0 baseline 391. Added: 8 dispatch-guidance cases + 41 agent-role-discipline cases. Two pre-existing smart-generator budget tests updated for the raised total (1200 → 2400 tokens; 4800 → 9600 chars).

### Removed from spec

The original 5.9.0 Tier B feature set (mnemos skill with three-tier memory + FTS5 + citation enforcement; brain skill with S0–S5 classifier CLI) was deprecated before any code shipped. Reasoning preserved in spec §0.5 — both features failed Filter 5 (autonomous trigger), added in this release as a hard pre-implementation check after retro-audit of `vibe:evolve` (5.8.0) showed the same anti-pattern. Mnemos returns when memory scale ≥ 100 entries OR rot-rate ≥ 5%; the brain classifier engine is reshaped here as the Feature 2 ambient guidance.

---

## 5.8.0 — 2026-05-02

First Tier A capability-boost release after 5.7.0's Tier S bundle. Three features deferred from 5.7.0's §7 / §0.5 lists, all augmenting the long-running CLAUDE.md / settings.json surface rather than adding new transient hook events. All three pass Iron Man Mandate, Audience Test, User Burden Zero, Capability Boost > Verification. Hook count stays 22; skill count 14 → 15 (one new — `vibe:evolve`). See `docs/spec/2026-05-02-vibe-5.8.0-tier-a.md` for the full spec.

### Changes

- **Feature 1 — `vibe:evolve` skill (ACE observer port).** New skill at `plugin/skills/evolve/` with three subcommands: `record <task_id> <score 0-1> <summary>` appends to `${CLAUDE_PROJECT_DIR}/.vibe/evolve_log.jsonl`; `reflect [--model <id>]` reads the log tail + the current managed block in `CLAUDE.md`, calls `claude -p` (default `sonnet-4-6`) with a STRICT-JSON delta prompt, parses the response, applies ADD/MODIFY/REMOVE deltas via a deterministic curator (cap 30 entries, oldest evicted on overflow), and splices the result back into `CLAUDE.md`; `revert` strips the managed block and decrements the version counter. Automatic rollback: if the rolling 5-entry score average after the previous edit drops by at least `VIBE_EVOLVE_ROLLBACK_DELTA` (default 0.05), the splice is undone and the managed block stripped — sign test only, cold-start safe (no prior window skips the check). Persistence: `.vibe/evolve-state.json` (version + last_score_before), `.vibe/evolve-history.jsonl` (per-event audit), `.vibe/claude-md-versions/` (timestamped CLAUDE.md snapshots). Bypass: `VIBE_NO_EVOLVE=1` env var, pause flag. Markers in `CLAUDE.md`: `<!-- VIBE:evolve-managed-start -->` / `<!-- VIBE:evolve-managed-end -->` placed **outside** the outer `<!-- VIBE:managed-* -->` envelope so `vibe:setup` re-renders never wipe evolve content. Source: `references/hone/src/hone/observer.py` (ACE pattern, GEPA dependency stripped — pure stdlib Python). Iron Man fit: compensates the static-CLAUDE.md weakness — the agent can now learn from accumulated outcomes across sessions instead of inheriting the same rules forever. Tests: 18 cases covering record/reflect/revert dispatch, malformed reflector JSON no-op, empty deltas no-op, 30-entry cap eviction, automatic rollback on score drop, cold-start safety.

- **Feature 2 — Git-signals managed block in `CLAUDE.md`.** New `{{GIT_SIGNALS_BLOCK}}` placeholder in `plugin/setup/claude-md-template.md` (between `Capability Audit` and `Verification Discipline` sections). `plugin/setup/smart-generator.sh` extended to emit a new top-level JSON key `git_signals` containing branch + base divergence + last-commit timestamp+subject + dirty count (staged/unstaged) + last 5 commit oneline. Re-rendered every `vibe:setup` and every `setup-check` run that hits a `MANAGED_REGION_PRESENT` CLAUDE.md. Per-block budget 800 chars; total budget raised 4800 → 5600 chars. Failure modes (silent skip, empty block emitted): non-git directory, `git` not on PATH, `VIBE_NO_GIT_SIGNALS=1` env var; detached HEAD renders as `detached at <sha>`; empty repo renders as `No commits yet`; commit count < 5 emits whatever exists. Iron Man fit: CLAUDE.md is read once at session start; surfacing volatile git state in a managed block keeps the agent calibrated against the actual repo state. Tests: 9 cases (clean main, dirty staged+unstaged, non-git dir, env-var bypass, empty repo, detached HEAD, commit count cap).

- **Feature 3 — Orphan-hook auto-recover (generalization of `detect-stale-hooks`).** Two new subcommands in `plugin/setup/reconciler.sh`: `detect-orphan-hooks <settings-path>` walks every hook entry (top-level legacy shape + nested canonical shape + `statusLine`) and token-scans each command — split on whitespace, skip tokens containing `${` or `$(` (templated), strip a single leading/trailing quote/backtick, then check `os.path.isfile` on absolute-path tokens. Flags any hook whose ANY absolute-path token is missing on disk. `apply-clean-orphan-hooks <settings-path>` (stdin: matches JSON) timestamps a `.bak-orphan-hooks-<ts>` backup, removes flagged hook entries, drops empty matchers and events. Integrated into `plugin/scripts/setup-check.sh` as Check 2c — runs in parallel with the existing 2b `detect-stale-hooks` denylist scan, same trigger and `VIBE_NO_AUTO_RECOVERY=1` bypass. Iron Man fit: CC blindly executes hooks listed in `settings.{,local.}json` and emits cryptic shell errors when a foreign-framework path stops existing — closes the gap in 5.6.3's auto-recover, which only matched the morpheus-v1 substring. Source: 5.7.0 spec §0.5 row F3 (Booost-app `morpheus/injector.sh` field signal). Tests: 16 cases (nested orphan, existing path NOT flagged, templated path skipped, mixed orphan+present, statusLine orphan, top-level legacy, invalid JSON unreadable, no-op apply on clean, idempotence, non-existent settings file).

### Skills

14 to 15. `vibe:evolve` is the new entry. Discoverable via the help skill and listed in `tests/run-tests.sh` `EXPECTED_SKILLS`.

### Hook count

Unchanged at 22. 5.8.0 adds capability INSIDE existing hooks (`setup-check` Check 2c) and a new skill, no new hook event registrations.

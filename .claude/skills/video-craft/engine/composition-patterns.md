...
Knowledge extracted from real SaaS/product videos. Used by video-craft when building scene configs to select appropriate layouts, transitions, and element arrangements based on narrative intent.
...
5. **Colors, fonts, spacing** come from the design system (ui-craft tokens), not from here

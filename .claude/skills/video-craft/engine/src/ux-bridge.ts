// Reads ui-craft design tokens from tokens.css (or legacy system.md)
...
function resolveDesignSystemPath(inputPath: string): string | null {
  ...
  // Also check .ui-craft/ subdirectory
  for (const name of ['tokens.css', 'system.md']) {
    const candidate = join(inputPath, '.ui-craft', name);
    if (existsSync(candidate)) return candidate;
  }
  return null;
}

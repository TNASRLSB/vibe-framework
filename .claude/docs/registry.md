...
| orson | `.claude/skills/orson/` | Video + audio production. Comandi: `/orson create`, `/orson render` (`--draft`/`--parallel`/`--no-audio`/`--preview`), `/orson demo`, `/orson batch`, `/orson formats`, `/orson entrances`. **v5.2**: voice intelligence (6 research-backed voice presets, locale-aware, WPM speech rate control), audio robustness (single -14 LUFS normalization, TTS retry + backoff, FFmpeg timeout, ducking param alignment), demo error recovery (action try-catch, overlay re-injection retry), subtitle improvements (word-wrap, position, empty validation), structured logging. **v5.1 aesthetic system**: 24 visual recipes, color arcs, camera motion, kinetic typography (`S()` helper), secondary animation, negative space rules. Claude scrive HTML direttamente (no autogen), animation runtime inline, component snippets library. PNG lossless, preview interattiva, parallel frame render, 136+ animazioni, 14 decorativi, 4 modi. HW encoding, draft mode, batch mode, asset embedding, PiP, SRT/VTT. Demo mode. Integrazione seurat + ghostwriter |
...
|----------|----------|-------|--------------|
| getAnimationRuntime | orson/engine/src/runtime.ts | - | v5: returns inline JS runtime string (easings, scene mgmt, __setFrame, S() text splitter) for Claude to embed in video HTML |
...
| renderParallel | orson/engine/src/parallel-render.ts | - | Renders frames in parallel Playwright workers then concat |
...
| selectEntranceByRole | orson/engine/src/actions.ts | - | Role-based semantic animation selection (reference catalog) |
| selectDecoratives | orson/engine/src/decorative.ts | - | CSS-only decorative elements (14 types) — reference for Claude's HTML |
| buildFrameChunks | orson/engine/src/parallel-render.ts | - | Splits totalFrames into N equal chunks for parallel rendering |
| previewHTML | orson/engine/src/index.ts | - | Serves HTML with player overlay on localhost for interactive preview |
...
| resolveVoicePreset | orson/engine/src/demo-script.ts | - | v5.2: resolves voice preset (voice, style, wpm, prosody) with locale override |
| wpm_to_rate | orson/engine/audio/narration_generator.py | - | v5.2: converts target WPM to Edge-TTS prosody rate string (baseline 160 WPM) |
| generateWebVTT | orson/engine/src/demo-subtitles.ts | - | v5.2: WebVTT generation with word-wrap, empty validation, position support |
| | | | |
